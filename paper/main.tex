...
\caption{The \method{} pipeline. Landmark extraction and depth
estimation run in parallel on the input photograph. A procedure-specific
mask is formed from the convex hull of relevant landmarks. The depth map
is modified with Gaussian displacement kernels (bridge flattening, tip
definition, side contrast) to encode the target surgical change.
FLUX.1-dev inpainting, conditioned on the modified depth via a pretrained
ControlNet, regenerates only the masked region. Pixels outside the mask
are copied verbatim from the input.}
\label{fig:pipeline}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\textwidth]{figures/fig3_decomposed_arcface.png}
\caption{Decomposed ArcFace similarity. Blue bars show automated
full-face ArcFace (Table~\ref{tab:main_results}); dark blue shows
per-example tuned results; green shows non-surgical region ArcFace.
The dashed line marks the 0.65 identity threshold. Non-surgical
scores near 1.0 confirm that inpainting preserves identity outside
the mask. Rhytidectomy achieves 0.982 with tuned parameters despite
0.173 in the automated pipeline, reflecting the sensitivity of
diffuse procedures to parameter selection.}
...
\caption{Effect of depth modification intensity $\alpha$ on
rhinoplasty prediction. Top row: predicted output at each intensity.
Bottom row: corresponding modified depth map. At $\alpha = 0$ the
prediction matches the input; increasing $\alpha$ produces
progressively more pronounced bridge flattening and tip definition.
This parameter provides clinically interpretable control over the
predicted surgical magnitude.}
\label{fig:intensity}
\end{figure}
